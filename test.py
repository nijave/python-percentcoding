...
TRUTH = [
  # name             flags      initargs    decoded   encoded  
  ...
  ("upper hex",      TEST_DECODE, [URI_SAFE,'A'], '*+,=>/', '%2A%2B%2C%3D%3E%2F'),
  ("lower hex",      TEST_DECODE, [URI_SAFE,'a'], '*+,=>/', '%2a%2b%2c%3d%3e%2f'),
]
...
def main():
  ...
  for name, flags, initargs, decoded, encoded in TRUTH:

    codec = percentcoding.Codec(*initargs)
